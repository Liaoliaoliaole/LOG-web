...
- The calibration popup includes a top shared layout (device info + metadata + summary) with unified row-style cards, a `Current Preview` area (`Measured raw value`, `Correct value`), `Read from SDAQ`, `Revert`, `Save to SDAQ`, live preview columns, and `Ctrl+S`.
