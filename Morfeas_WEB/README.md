...
- The calibration popup supports `Read from SDAQ`, `Revert`, `Save to SDAQ`, a top shared layout (device info + metadata + summary), a `Current Preview` block, live preview columns (`Calibrated value`, `Difference`), and `Ctrl+S`.
