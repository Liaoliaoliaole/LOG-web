...
\textbf{OPC UA mapped Value} below refers to the linked ISO channel \textbf{Value} node, not the raw device source node.

\begingroup
\small
\setlength{\tabcolsep}{2pt}
\begin{longtable}{|p{0.10\textwidth}|p{0.22\textwidth}|p{0.15\textwidth}|p{0.20\textwidth}|p{0.23\textwidth}|}
\hline
\textbf{Device} & \textbf{Status / Situation} & \textbf{Web Meas} & \textbf{OPC UA mapped Value} & \textbf{Meaning} \\
\hline
...
\hline
SDAQ & OFF-Line with runtime source missing & \texttt{---} when no logstat value is available & \texttt{-901} fallback & Linked SDAQ source node is missing; OPC UA still returns a numeric unavailable value. \\
\hline
IOBOX & Okay & actual value & actual value & Channel is valid and measuring normally. \\
\hline
IOBOX & OFF-Line & \texttt{-901} when core provides channel values; otherwise \texttt{---} & \texttt{-901} & IOBOX device or linked source is unavailable. \\
\hline
IOBOX & Disconnected & \texttt{-901} & \texttt{-901} & IOBOX is online, but the RX/channel path is not responding. \\
\hline
...
\hline
MTI & OFF-Line & \texttt{-901} when core provides channel values; otherwise \texttt{---} & \texttt{-901} & MTI device or linked source is unavailable. \\
\hline
MTI & Disconnected & \texttt{-901} & \texttt{-901} & MTI is online, but telemetry is not responding. \\
\hline
MTI & Disabled & \texttt{---} & \texttt{-901} fallback if the linked source node is missing & MTI telemetry was intentionally disabled by the operator; web does not invent a numeric value. \\
\hline
MTI & No sensor & \texttt{-902} & \texttt{-902} & MTI telemetry reports no sensor on the channel. \\
\hline
MTI & Unclassified & \texttt{-904} & \texttt{-904} & MTI telemetry is online, but the data is invalid. \\
\hline
NOX & Okay & actual value & actual value & UniNOx sensor is active and measuring normally. \\
\hline
NOX & Handler not running, NOX role removed, or sensor never detected & \texttt{---} & no value & NOX source node does not exist. This avoids phantom sensors. \\
\hline
NOX & OFF-Line after previously detected sensor timeout & \texttt{-901} & no value & Logstat records the timed-out sensor as offline; OPC UA source node is removed. \\
\hline
NOX & Heater off & \texttt{-901} & \texttt{-901} & Sensor is active, but heater-off state makes measurement unavailable. \\
\hline
...
\textbf{NOX mapped-channel note:} NOX uses dynamic sensor nodes for UniNOx addresses. If the NOX handler is not running, the bus has been switched away from NOX, or a sensor address has never been detected, the linked OPC UA channel has no source node and returns no value rather than a numeric error code. This is intentional and prevents phantom NOX sensors. In the web linker table, this appears as a simple unavailable measurement such as \textbf{Status = OFF-Line} with \textbf{Meas = ---}. If NOX is still configured and Morfeas core reports that a previously detected sensor has timed out, the same \textbf{OFF-Line} status may appear with \textbf{Meas = -901}. In short: \textbf{OFF-Line + -901} means core reported an offline NOX sensor; \textbf{OFF-Line + ---} means the NOX runtime source/logstat is no longer available for that link.
