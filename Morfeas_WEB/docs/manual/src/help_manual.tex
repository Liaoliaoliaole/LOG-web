...
\textbf{Web Status} below is the linker-table \textbf{Status} text. \textbf{Web Meas} comes from handler logstat files and usually represents interval data. \textbf{OPC UA mapped Value} refers to the linked ISO channel \textbf{Value} node used by downstream OPC UA clients.
...
\hline
\textbf{Device} & \textbf{Web Status / Situation} & \textbf{Web Meas} & \textbf{OPC UA Value} & \textbf{Meaning} \\
\hline
...
\textbf{NOX mapped-channel note:} NOX uses dynamic sensor nodes for UniNOx addresses. If the NOX handler is not running, the bus has been switched away from NOX, or a sensor address has never been detected, the web linker table may show \textbf{Status = OFF-Line} with \textbf{Meas = ---} because no logstat runtime source exists. The OPC UA mapped \textbf{Value} may return \texttt{-905} for the same ISO link because the mapped source node is missing. These two displays describe the same missing-source condition from different layers.
...
\item \textbf{Status = Unconfigured} with \textbf{Meas = ---} means the device is reachable, but the configured runtime channel path is not present.
...
In the current web interface, the standard replace action is offered only for eligible SDAQ channels. IOBOX, MTI, and NOX rows do not show the normal replace option.
