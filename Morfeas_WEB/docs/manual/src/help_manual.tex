...
\begin{itemize}
\item \textbf{Device Information}: serial number, device type, available channels, samplerate, maximum calibration points, and the fixed channel.
\item \textbf{Calibration Metadata}: calibration date, re-calibration period, channel unit, and used points.
\item \textbf{Point Table}: \texttt{Measure}, \texttt{Reference}, \texttt{Offset}, \texttt{Gain}, \texttt{C2}, and \texttt{C3}.
\item \textbf{Preview Columns}: \texttt{Corrected value}, \texttt{Difference}, and \texttt{Type}.
...
\screenshotplaceholder{Calibration Popup}{Insert a screenshot of the current calibration popup showing the metadata row, point table, summary panel, and Read / Revert / Save buttons.}
