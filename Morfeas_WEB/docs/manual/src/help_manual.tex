...
\hline
MTI & Radio Disabled / Disabled & \texttt{---} when logstat has no channel; otherwise \texttt{-901} if core stamped the old source & \texttt{-901} when old source node remains; \texttt{-905} if source node is gone & MTI telemetry was intentionally disabled or reconfigured. Web does not invent a numeric value. \\
\hline
...
\item \textbf{NOX}: search lists \textbf{NOx} and \textbf{O2} entries for detected UniNOx sensor objects. The current measurement does not need to be valid; for example, heater-off or warming-up sensors may still be linked and controlled later.
\end{itemize}
