...
function sdaq_load_anchor_map(string $jsonPath, array $xmlAnchors = []): array
{
    ...
    foreach ($data['SDAQs_data'] as $sdaq) {
        ...
        foreach ($measArr as $meas) {
            ...
            $map['channels'][] = [
                // Identity is always serial_anchor; the address fallback is
                // display-only while registration has not supplied a serial.
                'display_anchor'    => $serialAnchor ?? $sensorPathLower,
