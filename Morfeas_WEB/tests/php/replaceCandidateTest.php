...
// --- Fixture: one detected, unlinked IOBOX candidate ---
...
// --- XML: one existing IOBOX channel (replace source) and one existing SDAQ channel
//          with an unknown subtype (replace source for the regression test). ---
...
// --- 1) Successful Replace: client submits a non-canonical (lowercase) text that
//        resolves to the real candidate; the persisted ANCHOR must be the
//        candidate's own canonical form, never the client's literal text. ---
...
foreach ($xml->CHANNEL as $ch) {
    if ((string)$ch->ISO_CHANNEL === '_Old_IOBOX') {
        $written = (string)$ch->ANCHOR;
    }
}
check($written === '117440522.RX1.CH1', 'Replace persists the candidate\'s canonical anchor, not the client\'s literal text (got ' . var_export($written, true) . ')');

// --- 2) Regression test for the removed silent pass-through: an SDAQ source with
//        an unknown subtype and a target that matches NOTHING in the pool must be
//        rejected, not silently written verbatim (this used to "return" and let
//        iso_update_channel() persist the raw client anchor unmodified). ---
...
// --- 3) Family mismatch is still enforced after the refactor. ---
try {
    channel_replace_channel_from_pool(
        $xmlPath,
        '_Old_SDAQ',
...
// --- 4) Missing source ISO_CHANNEL is still rejected. ---
try {
    channel_replace_channel_from_pool(
        $xmlPath,
        '_Does_Not_Exist',
...
// --- Fixtures for the source-offline / field-allowlist tests below: two more
//     detected, unlinked IOBOX candidates. ---
$ioboxJsonLive = $dir . '/logstat_IOBOX_iobox2.json';
file_put_contents($ioboxJsonLive, json_encode([
    'Identifier' => '444444444',
    'IPv4_address' => '7.1.1.20',
    'Connection_status' => 'Okay',
    'RX1' => ['1' => 12.0, 'Status' => 1, 'Success' => 99],
]));
$ioboxJsonTarget = $dir . '/logstat_IOBOX_iobox3.json';
file_put_contents($ioboxJsonTarget, json_encode([
    'Identifier' => '333333333',
    'IPv4_address' => '7.1.1.30',
    'Connection_status' => 'Okay',
    'RX1' => ['1' => 8.0, 'Status' => 1, 'Success' => 97],
]));
$allIoboxLogs = [$ioboxJson, $ioboxJsonLive, $ioboxJsonTarget];

// --- XML: one channel whose anchor matches a currently-online candidate
//          (source-offline test), one with an empty INTERFACE_TYPE (family
//          test), and one legitimate offline channel to Replace with extra
//          smuggled fields (field-allowlist test). ---
$xml2 = simplexml_load_file($xmlPath);
$liveNode = $xml2->addChild('CHANNEL');
$liveNode->addChild('ISO_CHANNEL', '_Live_IOBOX');
$liveNode->addChild('INTERFACE_TYPE', 'IOBOX');
$liveNode->addChild('ANCHOR', '444444444.RX1.CH1'); // matches the online candidate above -> status Okay
...
$allowlistNode->addChild('ISO_CHANNEL', '_Old_IOBOX2');
$allowlistNode->addChild('INTERFACE_TYPE', 'IOBOX');
$allowlistNode->addChild('ANCHOR', '999999998.RX1.CH1'); // not in any logstat -> offline
$allowlistNode->addChild('DESCRIPTION', 'original description');
$allowlistNode->addChild('MIN', '-40');
$allowlistNode->addChild('MAX', '150');
$allowlistNode->addChild('UNIT', 'C');
file_put_contents($xmlPath, $xml2->asXML());

// --- 5) Source-offline re-verification (plan 5.4): a source that is
...
try {
    channel_replace_channel_from_pool(
        $xmlPath,
        '_Live_IOBOX',
        ['replace_mode' => true, 'anchor' => '333333333.RX1.CH1'],
        [], $allIoboxLogs, [], [], []
    );
...
// --- 6) Unresolvable source interface family must reject, not silently pass
//        the family-compatibility check. In the current row builder, a blank
//        INTERFACE_TYPE also always computes as status "Okay" (the catch-all
//        branch in channel_build_rows_with_logstat()), so this specific
//        malformed row is actually caught by the source-offline gate (test 5)
//        first; the dedicated replace_source_family_unknown guard is
//        defense-in-depth for any future code path that could leave a
//        channel's family unresolved while still reporting offline. Either
//        way, the row must never be replaceable, which is what this asserts.
try {
    channel_replace_channel_from_pool(
        $xmlPath,
        '_Blank_Type',
    ...
    check(
        in_array($e->apiCode(), ['replace_source_family_unknown', 'replace_source_not_offline'], true),
...
foreach ($xml3->CHANNEL as $ch) {
    if ((string)$ch->ISO_CHANNEL === '_Old_IOBOX2') {
        $after = $ch;
    }
}
check($after !== null, 'Replaced channel is still found under its original ISO_CHANNEL (iso_channel field was ignored)');
if ($after !== null) {
    check((string)$after->ANCHOR === '333333333.RX1.CH1', 'Replace still updates ANCHOR to the resolved candidate (got ' . (string)$after->ANCHOR . ')');
    ...
    check((string)$after->UNIT === 'C', 'Replace ignores a smuggled unit field (got ' . var_export((string)$after->UNIT, true) . ')');
}
$hackedFound = false;
foreach ($xml3->CHANNEL as $ch) {
