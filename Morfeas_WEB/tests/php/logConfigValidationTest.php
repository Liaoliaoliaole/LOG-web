 ...
 * validator built for FTP Restore. Covers exactly what the function
 * covers: real DTD structural validation (matching Core's own
 * Morfeas_XML_parsing(), which parses with XML_PARSE_DTDVALID), empty-leaf
 * rejection, DEV_NAME length boundary, and duplicate DEV_NAME/IPv4_ADDR
 * across IOBOX_HANDLER/MTI_HANDLER. Does NOT cover CAN ownership or
 * Disable-flag semantics -- out of scope, see the function's own doc
 * comment.
 *
...
// --- 6) DEV_NAME length boundary: Core's Dev_or_Bus_name_str_size
//        (IFNAMSIZ) is 16, so 15 bytes must be the last accepted length and
//        16 bytes must be the first rejected length. ---
...
try {
    ...
    check(false, "DEV_NAME of 16 bytes (Core's IFNAMSIZ) must throw");
} catch (ChannelConfigException $e) {
    check($e->apiCode() === 'invalid_device_name', "DEV_NAME of 16 bytes (Core's IFNAMSIZ) rejected with invalid_device_name (got " . $e->apiCode() . ')');
}
