...
\begin{itemize}
	...
	\item The node ``Calibration\_Data" must contain \textbf{only} the channels that will configured.
	\item Each channel node must contain ``Calibration\_date"", ``Calibration\_Period", ``Used\_Points", ``Unit" and ``Points".
