...
It's accepting POST and GET requests. At the following subsections the functionality for each type of request and the data that exchanged will be described in details.

\subsubsection{GET request}
The GET requests for this script requires two arguments (``SDAQnet" and ``SDAQaddr"). By this, will specified the name of the SDAQnet port and the current address of the SDAQ device that the request
is related. When the Morfeas\_SDAQnet\_proxy.php receive successfully this argument will attempt to get the calibration information of the SDAQ with the requested address that located on the requested SDAQnet.
If the device is available and answer at the request the script will forward the calibration data to the caller. 
The return of the script on success will be an XML object that caring out the calibration information of the device, an example of this shown at listing~\ref{lst:SDAQ_calib_info}. 
However, in case that some error occurred at the communication, the script will return a human readable message with the description of the failure.
Also, in case that this script called without the two required arguments, it will respond with ``HTTP ERROR 404".
...
<SDAQ>
  <SDAQ_info>
    ...
    <Type>Pseudo_SDAQ</Type>
    <Firmware_Rev>0</Firmware_Rev>
    <Hardware_Rev>0</Hardware_Rev>
    <Available_Channels>1</Available_Channels>
    <Samplerate>10</Samplerate>
    <Max_num_of_cal_points>16</Max_num_of_cal_points>
  </SDAQ_info>
  <Calibration_Data>
    <CH1>
